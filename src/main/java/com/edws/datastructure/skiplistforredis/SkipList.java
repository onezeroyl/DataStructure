...
public class SkipList<T extends Comparable<T>> {
    ...
    public SkipListNode<T> insert(double score, T data) {
        ...
        SkipListNode<T>[] previousArray = new SkipListNode[MAX_LEVEL];

        // span数组 - 记录到达每层previous[i]节点时已经跨越的节点数量
        // 用于计算和更新span（跨度）值，跨度表示forward指针跨越了多少个节点
        int[] spanArray = new int[MAX_LEVEL];
        ...
        for (int i = level - 1; i >= 0; i--) {
            ...
            spanArray[i] = (i == level - 1) ? 0 : spanArray[i + 1];
            ...
            while (current.getLevelList().get(i).getForward() != null && (
                ...
                spanArray[i] += current.getLevelList().get(i).getSpan();
        ...
        if (newLevel > level) {
            ...
            for (int i = level; i < newLevel; i++) {
                ...
                previousArray[i].getLevelList().get(i).setSpan((int) length);
            }
        ...
        for (int i = 0; i < newLevel; i++) {
            ...
            newNode.getLevelList().get(i)
                .setForward(previousArray[i].getLevelList().get(i).getForward());

            // 更新前驱节点第i层的forward指针指向新节点
            previousArray[i].getLevelList().get(i).setForward(newNode);

            // 更新跨度信息
            // 新节点第i层的跨度 = 原前驱节点的跨度 - (第0层到第i层的rank差值)
            // 这个计算确保了跨度的正确性，反映了forward指针实际跨越的节点数
            newNode.getLevelList().get(i).setSpan(
                previousArray[i].getLevelList().get(i).getSpan() - (spanArray[0] - spanArray[i]));

            // 前驱节点第i层的新跨度 = rank差值 + 1
            // +1是因为要跨越新插入的节点
            previousArray[i].getLevelList().get(i).setSpan(spanArray[0] - spanArray[i] + 1);
        }
        ...
        for (int i = newLevel; i < level; i++) {
            ...
            previousArray[i].getLevelList().get(i)
                .setSpan(previousArray[i].getLevelList().get(i).getSpan() + 1);
        }

        // 更新backward指针（反向指针）
        // 如果新节点的前驱是头节点，则新节点的previous为null
        // 否则指向真正的前驱节点，用于支持反向遍历
        newNode.setPrevious((previousArray[0] == header) ? null : previousArray[0]);
    ...
    public void print() {
        ...
        for (int i = level - 1; i >= 0; i--) {
            ...
            while (current != null) {
                // 打印节点的分数和数据，格式：[分数,数据]
                System.out.print("[" + current.getScore() + "," + current.getData() + "," + current.getLevelList().get(i).getSpan() + "] ");
